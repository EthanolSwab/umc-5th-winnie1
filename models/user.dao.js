...
export const addUser = async (data) => {
    try{
        ...
        //confirm 쿼리 수정 : 배열로 전달 data.email -> [data.email]
        ...
        //데이터베이스 쿼리에 대한 async/await 사용
        //prefer 데이터 추가 (JASON.stringify(data.prefer))
        const result = await pool.query(insertUserSql, [data.email, data.name, data.gender, data.birth, data.addr, data.specAddr, data.phone, JSON.stringify(data.prefer)]);
